...
function _hitbox_status($channel = false, $instance = false)
{
	...
	if(wp_remote_retrieve_response_code($hitbox) == '404'){
		if(get_option('sthw_hide_unknown') != '1'){
			return '<li class="st-hitbox-widget-list-item"><span class="st-hitbox-widget-title"><a target="_blank" href="http://www.hitbox.tv/' . $name . '">' . $name . '</a></span><span class="st-hitbox-widget-indicator">' . __('Unknown channel', 'sthw') . '</span></li>';
		}else{
			return false;
		}
	}elseif(wp_remote_retrieve_response_code($hitbox) != '200'){
		return '<li class="st-hitbox-widget-list-item"><span class="st-hitbox-widget-title"><a target="_blank" href="http://www.hitbox.tv/' . $name . '">' . $name . '</a></span><span class="st-hitbox-widget-indicator">'. __('Failed to connect to hitbox.tv api.', 'sthw') . '</span></li>'; 
	}
	...
	{
		$txt = '<span class="st-hitbox-widget-title"><a target="_blank" href="http://www.hitbox.tv/' . $name . '">' . $stream['livestream'][0]['media_user_name'] . '</a></span>';
		$txt .= '<span class="st-hitbox-widget-indicator">' . __('Offline', 'sthw') . '</span>';
	}
